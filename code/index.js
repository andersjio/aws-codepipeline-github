...
function downloadSiteFromGithub(repoConfig, proxy){
    console.log("Downloading master zip of " + repo + " from GitHub")
...
async function downloadFileFromGithub(repoConfig, proxy) {
    ...
    const params = {
        ...
        url:  `${repoConfig.projectName}/${repoConfig.repoName}/zipball/${repoConfig.branch}`,
        responseType: 'stream',
        headers: {
                "Authorization": `token ${repoConfig.token}`,
                "User-Agent": `andersjio`
            }
        }
    };
